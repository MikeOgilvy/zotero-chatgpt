...
- 在本仓库继续。先读 [产品规格](docs/zotero-codex-user-flow.md)、[架构契约](docs/module-design.md)、[当前进度与计划](docs/progress.md)。开发/宿主/发行命令在 [development](docs/development.md)。不再使用历史 S/T 阶段计划作为授权范围。
...
- 同一文件一个写入负责人。保护已有未提交改动，不自动提交；不 push、公开发布、启用付费服务或操作真实文献库。用途不明或未提交的待清理内容保留并记入阻塞。
...
- 宿主测试仅 `.zcr-dev/` 专用 profile/data 与合成 PDF。保护正常 Zotero、已有开发 profile 和其他 Codex 会话。测试脚本默认可能自动执行：按开发文档明确选择模式。
