...
export function validateRevision(value: unknown): DocumentRevision {
  const rev = record(value, ['fingerprint', 'size', 'modifiedAt', 'sha256']);
  if (rev.sha256 !== undefined && (typeof rev.sha256 !== 'string' || !/^[a-f0-9]{64}$/u.test(rev.sha256))) invalid();
  if (!Number.isSafeInteger(rev.size) || Number(rev.size) < 0 || !Number.isSafeInteger(rev.modifiedAt) || Number(rev.modifiedAt) < 0) invalid();
  return { fingerprint: label(rev.fingerprint, 256), size: Number(rev.size), modifiedAt: Number(rev.modifiedAt), ...(rev.sha256 !== undefined ? { sha256: rev.sha256 } : {}) };
}
export function validateDocument(value: unknown): DocumentContext {
  const doc = record(value, ['id', 'paper', 'revision', 'parserVersion', 'totalPages', 'pages', 'sourceId']);
  const id = label(doc.id, 36);
  if (!/^[a-f0-9]{8}-(?:[a-f0-9]{4}-){3}[a-f0-9]{12}$/u.test(id)) invalid();
  const revision = validateRevision(doc.revision);
  if (doc.sourceId !== undefined && (typeof doc.sourceId !== 'string' || !/^[a-f0-9]{8}-(?:[a-f0-9]{4}-){3}[a-f0-9]{12}$/u.test(doc.sourceId))) invalid();
...
export function documentSummary(doc: DocumentContext): DocumentSummary {
  return { id: doc.id, revision: { ...doc.revision }, parserVersion: doc.parserVersion, totalPages: doc.totalPages,
    ...
    textBytes: doc.pages.reduce((bytes, page) => bytes + new TextEncoder().encode(page.text).length, 0), ...(doc.sourceId ? { sourceId: doc.sourceId } : {}) };
}
