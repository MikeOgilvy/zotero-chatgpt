...
- Verified in this working tree: `npm run typecheck` and `npm run lint` pass; `npm run test:unit` is **631 tests / 57 files**; `npm run package:dev` produces `dist/zotero-codex-reader-0.4.0a1-dev.xpi`; `npm run verify:artifacts` passes with **77 files** (digest in `dist/SHA256SUMS`). A clean-checkout rebuild (temporary `git worktree` at HEAD + `npm ci`) reproduced typecheck, 631 unit tests, `package:dev`, and `verify:artifacts` (**77 files**) with the identical SHA-256 digest; the pinned runtime binary was reused from the local cache rather than re-downloaded.
