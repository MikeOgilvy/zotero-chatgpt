...
日期：2026-09-08。状态：名称、技术路线和开发流程已收敛；尚未开始插件实现。本文取代早期 Node companion + HTTP 中转的技术假设，用户交互要求保持不变。
...
| 首个测试版本 | `0.1.0-alpha.1`；通过发行验收后再发布 `0.1.0` |
| 仓库状态 | 名称已选定，尚未创建远程仓库，也不声称名称独占或已注册 |

这个名称同时说明宿主、后端和使用场景，适合 GitHub 搜索、README 与用户安装列表。仓库名固定，GitHub owner 在实际创建仓库时选定；扩展 UUID 在 T1/T2 生成后保持稳定，不随仓库地址变动。
...
源码已经确认能力入口，但还不能据此声称完整插件可用。T0 优先验证：

1. 实际下载的 XPI 提取并启动匹配架构的 Codex，路径含中文/空格时正确，操作系统安全检查可正常通过。
2. UTF-8 拆包、逐行 JSON、EOF、取消和进程退出全部可控；不把一次 pipe read 当成一条消息。
3. 插件内发起官方浏览器登录，独立账户状态不影响开发用 Codex。
4. 文件快照原子替换和请求日志的持久化语义可支撑不确定提交恢复；不能靠函数名推断落盘保证。
5. 模型、速度、推理强度在同一个对话下一轮生效。
...
先完成 S0 仓库/工程初始化与 S1 真实 Zotero 外壳，然后进入 S2 的自带 Codex/官方登录。最小去重和提交记录在真实发送前建立，T0 的对应验证穿插进行。当前仍为计划状态，不把文档完成计为插件功能完成。
