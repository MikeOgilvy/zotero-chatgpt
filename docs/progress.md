...
- **本轮门禁（0.4.0a8，同一树、按序）**：`npm run typecheck` PASS；`npm run lint` PASS；`npm run test:unit` **1067 passed / 79 files / 0 skipped**；`npm run package:dev` → `dist/zotero-codex-reader-0.4.0a8-dev.xpi`（92,675,778 bytes，SHA-256 `896063bf1078e46b06577cb027668033aca7fd4bc4d92923a86744bf727a6529`）；`npm run verify:artifacts` **84 files PASS**；打包后复跑 `test:unit` 同为 1067 / 79 / 0 skipped。以上均为代码 + 单元 + 产物证据；**真实宿主、真实模型 NOT RUN**。

### 产物边界

| 字节（SHA-256） | 版本 | 位置 | 证据层次 |
| --- | --- | --- | --- |
| `896063bf1078e46b06577cb027668033aca7fd4bc4d92923a86744bf727a6529`（92,675,778 B） | 0.4.0a8 | `dist/`（`SHA256SUMS` 唯一条目） | 代码 + 单元 + 产物；**无宿主证据，从未安装** |
...
- 本机（macOS，`dist/` 存在当前 manifest 版本的 XPI）：**1067 passed / 79 files / 0 skipped**（2026-09-15 a8 树，打包后复跑）；打包前同树为 1065 passed / 2 skipped，差额即 `install-lifecycle.test.ts` 的两条 `it.skipIf`。相对 a7 的 1076/80 净 −9 / −1 file：删除两列只读转录（`pane-layout` 与对应 dock 用例）。
