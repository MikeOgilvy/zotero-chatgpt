...
- **0.4.0a13 的真实宿主状态**：Stage 8 打包的 a13 XPI 已装进 `.zotero-chatgpt-dev/context` 树并跑通 `--context` **32/32 PASS**、`recordedRequests = 0`（见上“打包与宿主验证回合（0.4.0a13）”）。它证明的是 **a13 字节加载后当前 PDF 本地读取/会话/偏好路径仍可用，不是 Stage 8 的模式分离本身**：该冻结 driver **不点击 Chat/Agent 模式控件**、不做写操作，故模式控件在真实 dock 的布局/命中区、真实 IME、焦点、滚动锚点与原生缩放仍**未观察**；Chat 模式“结构性只读”未在真实 Zotero/真实库中尝试写操作来证实。**真实模型（`--live`/`--live-model`）与 `--context --native` NOT RUN**。
- **0.4.0a12 的真实宿主状态（历史，已被 a13 取代）**：`--context` **已跑 32/32 PASS**、`recordedRequests = 0`（见上“打包与宿主验证回合”），当时该 XPI 已装进 `.zotero-chatgpt-dev/context` 树；该树现为 a13。`--context --native` NOT RUN。该 frozen driver **不点击 Chat/Agent 模式控件**，故 Stage 6 模式控件在真实 dock 的布局/命中区、真实 IME、焦点、滚动锚点与原生缩放仍**未观察**；Chat 模式“结构性只读”未在真实 Zotero/真实库中尝试写操作来证实。**真实模型（`--live`/`--live-model`）NOT RUN**。
