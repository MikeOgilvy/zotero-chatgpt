...
- 本机（macOS，且 `dist/` 存在**当前 manifest 版本**即 a5 对应的 XPI）实测：**`npm run test:unit` → 1044 passed / 79 files / 0 skipped**（2026-09-14 侧栏/元数据/PDF 回合实测）。
- 与上一版 990 / 77 的差异来自本轮：新增纸张书目卡片与本地读取状态视图用例、PDF 就绪上限回归（`document-version.test.ts`）、历史直删与偏好面板文案更新，删除死模块后的 `tests/core/workspace-export.test.ts` 一并移除。
