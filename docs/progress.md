...
- 本机（macOS，且 `dist/` 存在当前 manifest 版本对应的 XPI）实测：**`npm run test:unit` → 988 passed / 77 files / 0 skipped**。
- 去掉该 XPI（例如干净 checkout）实测：**986 passed / 77 files / 2 skipped**。
...
- CI 说明：CI 打出来的 XPI 是 **darwin/arm64 产物、但在 linux 上构建、且从不宿主执行**。因此第 1 条只有在“打包后同作业再跑一次 `test:unit`”时才会真正执行；第 2 条在 linux runner 上**始终 skip**（无 `codesign`，也非 darwin）。以上 988/986 均为**代码 + 单元**证据，不是宿主或真实模型结论。
...
| 草稿持久化：**文档 vs 代码矛盾（未判定，需宿主判定）** | `docs/progress.md` 第 98 行“人工试用”段称**“未发送草稿目前仅在插件寿命内保留，重启不会恢复”**；但 `packages/zotero/src/chat/presenter.ts` 的 `loadLocal`→`workspace.readDraft`（约 230-258 行）与 `stageDraft`/`flushDraft`→`workspace.saveDraft`（约 260-288 行），加上 `packages/core/src/workspace/store.ts` 的 `draftPath`/`loadDraft`/`saveDraft`/`readDraft`（把草稿写进 `workspace/drafts/*.json` 持久化存储，约 191-246 行），**看起来会持久化并在重启后恢复**。**不猜测哪一侧正确、不静默删改任一侧**；列为宿主验证项：在专用 profile 重启后检查未发送草稿是否恢复，以判定“哪个为真” | F |
| 许可/第三方声明缺口（**下一阶段，本轮故意不修**） | `content/zcr.js` 打包进了 `linkify-it`、`mdurl`、`uc.micro`、`punycode`（均 MIT）与 `entities`（BSD-2-Clause），但随包只带 `dompurify`（MPL-2.0 OR Apache-2.0）/`katex`/`markdown-it` 的 LICENSE。根因：`scripts/build.mjs` 的 `copyThirdPartyAssets` **约 77-86 行**的 `cp` 列表只拷这三个。需补齐这 **5 个缺失声明**（`linkify-it`、`mdurl`、`uc.micro`、`punycode`、`entities`）；`runtime/README.md` 第 7 行也已承认 Rust 侧“A complete release dependency/license audit remains an S6 task”。**本轮不修**，因为改动会改变随包字节、让 `dist` 第三次漂移（见『产物边界（重要）』的版本号约束） | F |
