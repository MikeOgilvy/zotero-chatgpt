...
## 仓库整理（2026-09-15，分支 `cursor/repo-cleanup-becf`，未 push）

基线 `f48f337`（`main`，工作树干净）。目标是只保留核心：Zotero 原生界面 → TypeScript core → Gecko stdio → 随包 Codex App Server 的源码、当前有效的四份文档与其配套脚本/测试/CI。每个可验证变更一个本地提交；未 push、未发布、未动真实文献库或任何认证文件。

- **删除的文档**：`docs/archive/progress-history-2026-09.md`（2026-09 迭代流水）。仍有用的信息已迁入本页 [历史](#历史)：可恢复旧原文的基线提交、已归档宿主报告的磁盘位置、0.4 之前 S1–S6 的证据摘要。原文逐字保留在 `f48f337`。
- **删除的测试驱动**：`tests/host/driver.js`（S1）、`s2-driver.js`、`s3-driver.js`、`s4-driver.js`。判定依据：它们断言的 UI 钩子（`item-details.zcr-chat-active`、`data-zcr-output`、`data-zcr-input`、`data-zcr-selection-bar`、`data-zcr-picker-menu`、`data-zcr-context-title`、`.zcr-menu` 等）在 `packages/zotero/` 中零命中，属被 in-reader dock 取代的 context-pane 时代产物，对任何当前 XPI 都不可能通过。`s5-driver.js`/`s6-driver.js` 使用的钩子（`data-zcr-toggle`、`data-zcr-chat`、`zcrRuntime`/`zcrAuth`/`zcrConversation`/`zcrGenerating`、`data-zcr-action="login"`）仍在产品中，保留。随之 `scripts/host-test-stage.mjs` 只登记 `s5`/`s6`/`context`/`live-model`，**不再有隐式默认阶段**（无参数即拒绝，避免误写 profile），`--login`（仅 s2/s3 读取）一并移除；`tests/build/host-test-stage.test.ts`、`prepare-acceptance.test.ts` 与 development 同步。
- **删除的死代码**（零调用、零测试）：`chat/pick-images.ts` 的 Gecko FilePicker 路径（`ImageFilePicker`、`pickImageAttachments`、`geckoPickImageFiles` 及其 `ChromeUtils`/`IOUtils` 声明；实际的附件选择走 `reader/library.ts` 的 `LibraryFilePicker`）、`chat/text-scale.ts` 的 `chatScaleFromReaderZoom`、`core/context/bibliography.ts` 的 `BIBLIOGRAPHY_LABELS` 别名、`core/workspace/history.ts` 的 `parseHistoryListing`/`parseHistoryReport`。
- **删除的遗留功能写路径**：archive/restore（旧归档功能早已从侧栏和偏好面板移除，此前记录为“待侧栏补丁一并清理”）。移除 `ReaderClient.archiveConversation`、`ReaderService.archiveConversation`、`ReaderCoreClient.archiveConversation`、`ReaderWorkspace/HistorySource.setConversationArchived`、`WorkspaceStore.setConversationArchived`、`HistoryManager.setArchived/setArchivedByIds/confirmScope`，`HistoryAction` 收窄为 `'delete'`，`isHistoryReport` 只接受 `delete`。**读路径不变**：旧记录的 `archivedAt` 仍被校验、按 `HistoryScope` 列出并当作普通会话显示，任何记录都不被重写；`HistoryFilterScope`/`historyCounts`/`activeCount+archivedCount` 因此保留。
- **文档/配置更正**：development.md 与 module-design.md 的版本字面量 a5 → a6（与 manifest 一致）；CONTRIBUTING.md 关于 “packaging 不在 Ubuntu CI” 的陈述已过时，改为与 `ci.yml` 一致；`.cursor/install.sh` 注释里的 0.144.1 → 0.154.0；本页“安装状态”一行中重新泄露的 owner 真实 profile 目录名按 `18e2770` 的隐私约定替换为占位符。
- **保留并记录（未删）**：`runtime/README.md` 末句 “license audit remains an S6 task” 是对固定运行资产的独立声明，未动；`packages/contracts/src/index.ts` 两处注释引用 `rust-v0.144.1` 是协议形状的出处，未动；`packages/zotero/src/reader/metadata.ts` 是有意的分层入口（core 不得依赖 zotero），有生产导入，保留；一批只被单元测试引用的导出（如 `bibliographyView`、`resolveAllowedModels`、`historyCounts`、`imagesFromClipboardItems`、`MIN_TWO_COLUMN_WIDTH`、`contextUsageLabel`）是测试可见的纯函数，非死代码，保留；`.github/` 模板、`.cursor/` 环境配置、`tests/host/s5-driver.js`（s5 的 leftover 注入只认 schema 1，schema 3 记录会走 skip 分支——功能仍成立，但覆盖减弱，未改）保留。`.gitignore` 已覆盖 `.DS_Store`、`.superpowers/`、`.worktrees/`、`dist/`、`build/`、`.zcr-dev/`，均未被跟踪，无需改动。
- **本轮门禁（同一树、按序，代码+单元+产物）**：`npm run typecheck` PASS；`npm run lint` PASS；`npm run test:unit` **1077 passed / 80 files / 0 skipped**（打包后复跑；打包前 1075/80，差额为 `install-lifecycle` 两条以“当前版本 XPI 存在”为条件的用例）；`npm run package:dev` → `dist/zotero-codex-reader-0.4.0a6-dev.xpi`（**92,682,070 bytes**，SHA-256 `61225f4e1a74a584de5ecf579f29ca4a8376470de372c0a2bb10d15038415c07`）；`npm run verify:artifacts` **84 files PASS**。与整理前 1081/80 的差异：删除 `client.test.ts` 的 archive 用例 1 条、`history-management.test.ts` 的 archive 用例 2 条、`prepare-acceptance.test.ts` 的 `--login` 用例 1 条；`host-test-stage.test.ts` 的默认阶段用例改为“无阶段即拒绝”。**真实宿主、真实模型未跑**；本轮产物 `61225f4e…` 没有任何宿主证据。
- **一次误操作（已恢复到可恢复的程度，如实记录）**：为观察“无阶段即拒绝”的失败回归，曾在实现前直接执行了 `node scripts/prepare-host-test.mjs`（无参数）。旧行为默认 S1 阶段，于是它在 `.zcr-dev/profile`（已登录开发 profile，最后使用 2026-09-10）上：重写 `user.js`（标准测试 prefs）、把 `extensions/{8a5f5bde-…}.xpi` 从原 `0.3.0a1`（据 `extensions.json`）替换为当时 `dist/` 的 a6（`bc9d1370…`）、写入 S1 驱动 `extensions/zcr-host-test@local.xpi`、重写 `.zcr-dev/fixtures/reading.pdf`（确定性合成 fixture）。执行前脚本已确认没有 Zotero 实例在用该 profile。补救：立即删除新写入的驱动 XPI（避免下次启动自动跑已删除的 S1 驱动）；被替换的 0.3.0a1 字节无备份、不可恢复（可从 tag `v0.3.0a1` 重打包）；`records/`、`account/` 未被触碰、未读取。此事件正是把默认阶段改成显式拒绝的直接理由；此后的失败回归改用纯模块 `selectHostStage([])` 观察，无副作用。
...
- **本轮门禁（同一树、按序，代码+单元）**：`npm run typecheck` PASS；`npm run lint` PASS；`npm run test:unit` **1068 passed / 79 files / 0 skipped**（见 [测试计数（唯一权威）](#测试计数唯一权威2026-09-15-实测)）。**未跑 `package:dev`/`verify:artifacts`、真实宿主、真实模型**（本轮按任务限定禁止打包与宿主命令），故 `dist/` 仍是上一轮的字节，本轮代码无任何产物或宿主证据。
...
- **本轮门禁（2026-09-14 两列只读转录回合，同一树、按序，代码+单元）**：`npm run typecheck` PASS；`npm run lint` PASS；`npm run test:unit` **1081 passed / 80 files / 0 skipped**（见 [测试计数（唯一权威）](#测试计数唯一权威2026-09-15-实测)）。**未跑 `package:dev`/`verify:artifacts`、真实宿主、真实模型**（本轮任务明确禁止打包与宿主命令），故 `dist/` 仍是上一轮的字节，本轮代码无任何产物或宿主证据。
...
- **安装状态（2026-09-14 09:03 只读核对）**：owner 正常 profile（`<owner profile>`（真实 profile 目录名已按 `18e2770` 的隐私约定不写入仓库））当前装的是 **`0.4.0a5`**：`extensions/{8a5f5bde-b4e1-41eb-b5d9-2774afa0cf72}.xpi` **92,665,647 bytes**、SHA-256 `68529c36cfd422268a7f24fc05eaf350b057c1457923d1f8320a6bdc7129261b`，与 `dist/zotero-codex-reader-0.4.0a5-dev.xpi` **逐字节一致**（`cmp` 相同）；`extensions.json` 报 addon version **`0.4.0a5`**（`active: true`，`sourceURI` 指向 `dist/` 的 a5）。mtime：XPI / `extensions.json` / `addonStartup.json.lz4` 均为 `2026-09-14 08:33`，`prefs.js` `08:35`；`prefs.js` **不含** `extensions.startupScanScopes`。替换 a4 时**没有**留下 a4 的当时备份（profile 侧最新只到 `…zcr-bak-20260913-215714-0.4.0a3`）；宿主已验证的 a4 字节仍保留在 `.zcr-dev/artifact-backup/zotero-codex-reader-0.4.0a4-dev.host-verified-5a6bb161.xpi`（`5a6bb161…`），profile 侧回退备份已于本轮补齐（见 [产物边界（重要）](#产物边界重要)）。**这是 profile 变更、不是仓库提交，其本身也不构成宿主验证证据**；没有任何 a5 在 owner 真实文献库被目视/使用的证据。**（本轮补注：该 09:03 快照当时成立；此后 `package:dev` 重建使 `dist/` 的 a5 变为 `426542c2…`，profile 装的仍是 `68529c36…`，两者不再逐字节一致，见 [产物边界（重要）](#产物边界重要)。）**

### 产物边界（重要）

- **canonical `dist` 产物当前 = `dist/zotero-codex-reader-0.4.0a6-dev.xpi`（92,682,070 bytes，SHA-256 `61225f4e1a74a584de5ecf579f29ca4a8376470de372c0a2bb10d15038415c07`）**，由 2026-09-15 仓库整理分支的 `package:dev` 打出（含 pick-images/text-scale/history 死代码删除与 archive 写路径移除），`verify:artifacts` **84 files PASS**；**尚未经任何宿主验证**，也从未安装进 owner 的正常 profile。整理前的 a6 字节（92,682,832 bytes，SHA-256 `bc9d1370…`，两列只读转录回合 HEAD 打出，同样未宿主验证）已被此次重建覆盖，目前只残留在 `.zcr-dev/profile/extensions/` 中（见“仓库整理”一节的误操作记录）。以下关于 a6 的段落写于整理前，描述的是 `bc9d1370…` 那份。产物内已核验：`manifest.json` 声明 `0.4.0a6`，`content/zcr.js` 含本回合新符号（`zcr-pane-preview`、`zcr-workspace-search`、`readableColumnCount`、`attach-file`）。**提升版本号的意义**：a5 已被同版本不同字节污染（下述 (d)），Zotero 可能因此拿到陈旧的 `jar:` 句柄而在升级 XPI 后仍显示旧界面；a6 是新的版本串，升级后不必依赖清理陈旧资源。**本产物携带的是 `8840253`（产出 canonical a5 那一轮的文档提交）之后的 15 个提交**（`3cec949`…`f1e0dff`，即今天全部侧栏/附件/剪贴板/引用搜索/多会话/两列只读的改动）；此前 `dist/` 里的 a5（`426542c2…`，14:49 打出）**早于这 15 个提交中的每一个**，所以在此之前无法在真实 Zotero 里看到今天的任何一条修复。
...
### 测试计数（唯一权威，2026-09-15 实测）

本页**只有这一处**声明当前测试计数；其它出现过的数字（699/60、746/67、970/76、968+2、629+2 等）都是更早迭代的历史值，只保留在 Git 历史中（见 [历史](#历史)），不再作为当前计数。

- 本机（macOS，且 `dist/` 存在**当前 manifest 版本**即 a6 对应的 XPI）实测：**`npm run test:unit` → 1077 passed / 80 files / 0 skipped**（2026-09-15 仓库整理后的代码，在 `package:dev` 打出 `dist/zotero-codex-reader-0.4.0a6-dev.xpi` 之后复测；打包前同树为 1075 / 80）。
- 与整理前 1081 / 80 的差异（−4）全部来自删除：`tests/core/client.test.ts` archive/restore 用例 1 条、`tests/core/history-management.test.ts` archive 用例 2 条、`tests/build/prepare-acceptance.test.ts` `--login` 用例 1 条；`tests/build/host-test-stage.test.ts` 的“默认 S1”用例改写为“无阶段即拒绝”（数量不变）。1081 / 80 是 2026-09-14 两列只读转录回合的历史值。
...
开始时 main / HEAD `1fdd3dc`，56 个已有修改/未跟踪文件。未提交、未推送。原内容与 diff 保存在忽略的 `.zcr-dev/verification/scope-2026-09-11/baseline/`；当前 diff 包含这些既有工作，不能全部归为本轮新增。**下表是 2026-09-13 那轮迭代的对照记录；其中的 `test:unit` 数字均已由 [测试计数（唯一权威）](#测试计数唯一权威2026-09-15-实测) 取代，仅作历史。**

| 检查 | 本轮开始 | 当前结果与边界 |
| --- | --- | --- |
| `npm run typecheck` / `npm run lint` | PASS | PASS |
| `npm run test:unit` | 305 PASS / 1 FAIL，39 files | **见 [测试计数（唯一权威）](#测试计数唯一权威2026-09-15-实测)**；早期 699 PASS / 60 files 等历史数字见 Git 历史（[历史](#历史)） |
...
| 临时 `git worktree` + `npm ci` 的 clean HEAD 重建 | 未执行 | **PASS**：typecheck PASS；`test:unit` 见 [测试计数（唯一权威）](#测试计数唯一权威2026-09-15-实测)（无 `dist/` 时 2 条 skip）；`package:dev` → 0.4.0a1 XPI；`verify:artifacts` 77 files，digest 与主树一致；复用本地固定 runtime 缓存，未重新下载 |
...
- [x] 合并 `feat/usage-batch-1` 与 `fix/audit-bugs-ui` 并补完耗时 UI（代码+单元）：两侧测试均保留，全量 `test:unit` 连续两次通过（计数见 [测试计数](#测试计数唯一权威2026-09-15-实测)），重打包 `verify:artifacts` **77 files**；真实宿主/模型仍未重跑。
...
- [x] 0.4.0a4 版本提升、门禁、打包与产物校验（代码+单元+产物）：提交 `96f8c2c`；`typecheck`/`lint` PASS、`test:unit` PASS（计数见 [测试计数](#测试计数唯一权威2026-09-15-实测)；该轮历史数字见 Git 历史）、`package:dev` → `dist/zotero-codex-reader-0.4.0a4-dev.xpi`（92,661,563 bytes，SHA-256 `5a6bb161…`）、`verify:artifacts` 79 files PASS；owner profile 已装入 a4 供验收（profile 变更，非仓库提交）。
...
- [x] 死代码清理（代码+单元，2026-09-13）：移除 `sidebar.css` 中已无引用的 `.zcr-history-archived-label` 规则，并删掉 `sidebar-styles.test.ts` fixture 中同 class 的无断言 span；提交 `f1e8044`，清理后全量通过（计数见 [测试计数](#测试计数唯一权威2026-09-15-实测)）。
- [x] 开发 XPI 安装流自校验（代码+单元+只读真实 profile，2026-09-13）：新增 `npm run install:dev`（`plan`/`install`/`check`/`revert`/`rollback`）与 18 条单元/临时树回归（`test:unit` 计数见 [测试计数](#测试计数唯一权威2026-09-15-实测)），脚枪与命令见 development；真机 `check` 仍需一次目标 profile 重启才能证明，owner 当前实例按决定未重启。
...
- 0.4 之前的宿主证据（旧外壳、旧包，不继承到当前 dock/样式/发行；只说明当时验证过什么）：S1 外壳/启停 27/27（2026-09-08 早期 shell 包）；S2 原生 runtime 12/12 executed、3 NOT RUN（2026-09-09，真实 turn 被 typed quota 拒绝）；S3 选区 23/23、2 NOT RUN；S4 交互 40/40、5 NOT RUN（包 `c1b898ac…`）；S5 恢复 17/17、1 NOT RUN（同包）；S6 virgin 15/15、a1→a2→a1 19/19（a2 `445f4724…`）。其中 S1–S4 驱动针对的旧 UI 钩子（`item-details.zcr-chat-active`、`data-zcr-output`、`data-zcr-input`、`data-zcr-selection-bar`、`data-zcr-picker-menu`、`data-zcr-context-title` 等）已不在产品中，四个驱动文件（`tests/host/driver.js`、`s2-driver.js`、`s3-driver.js`、`s4-driver.js`）随 2026-09-15 整理删除；S5/S6 驱动使用的 `data-zcr-toggle`/`data-zcr-chat`/`zcrRuntime`/`zcrAuth`/`zcrConversation`/`zcrGenerating` 钩子仍在产品中而保留。
