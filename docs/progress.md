...
- **本轮门禁（Stage 8，代码树 `0a02638`，2026-09-18）**：`npm run typecheck` PASS；`npm run lint` PASS；`npm run test:unit` **82 files / 1139 passed / 0 skipped**（5.3s）。相对 Stage 6/7 的 1092 为 **+47 用例 / +3 文件**：新增 `tests/core/action-intent.test.ts`（30）、`tests/zotero/chat/chat-execution.test.ts`（5）、`tests/zotero/chat/agent-execution.test.ts`（4），`tests/zotero/chat/presenter-workspace.test.ts` **+7**（模式执行分离验收），`tests/build/dependency-boundaries.test.ts` **+1**（Chat 执行路径边界）。回归先红后绿：`chat/chat-execution.ts` 与 `refreshTaskState(`mode==='agent')` 落地前，新验收中 5 条失败（Chat 触达 Agent 端口、自然语言写指令未被拒、多轮阅读静默升级），落地后全绿。**本次未运行** `npm run package:dev` / `npm run verify:artifacts` / `install:dev` / `verify:install` / `release:dry-run` 与任何宿主驱动，故 `dist/` 与 0.4.0a12 字节不变、无新 XPI。
...
| --- | --- | --- | --- |
| `b1677ecd0f2ad525c6dc5734dcd78a12aee9fc1987e57019c204717d7aa157cb`（92,679,180 B） | 0.4.0a12 | `dist/`（`SHA256SUMS` 现列 a12；a11 的 `dist/` 副本仍在磁盘但已不被收录） | 代码 + 单元 + 产物 + **真实宿主 `--context` 32/32 PASS**（2026-09-18，专用 `.zotero-chatgpt-dev/context` 树，`recordedRequests = 0`）：7 阶段 Chat/Agent 重构后首次打包、首次装入宿主树。**未装进 owner 正常 profile**；**真实模型 NOT RUN** |
...
- 本机（macOS，代码树 `0a02638`，`dist/` 现为 0.4.0a12 XPI，2026-09-18）：**1139 passed / 82 files / 0 skipped**（本机 `dist/` 有当前 manifest 版本 XPI，`tests/build/install-lifecycle.test.ts` 的两条 `it.skipIf` 均执行；本轮**未**在打包前复跑，故不声明 −2/+2 的数字，规则见下条）。相对 Stage 6/7 代码树 `75fc5a9` 的 **1092 passed / 79 files** 为 **+47 用例 / +3 文件**，全部是新增、无删除：`tests/core/action-intent.test.ts` +30、`tests/zotero/chat/chat-execution.test.ts` +5、`tests/zotero/chat/agent-execution.test.ts` +4、`tests/zotero/chat/presenter-workspace.test.ts` +7（模式执行分离验收）、`tests/build/dependency-boundaries.test.ts` +1（Chat 执行路径边界）。同文件内「long-source reading coordinator」「persisted reading jobs」两条是**改写**为显式 Agent 档（多轮阅读现为 Agent 动作），非删除覆盖。
...
依据 owner 本轮的明确任务与[仓库重构计划](repository-refactor-plan.md) 的 R1/R2 方向，在 Stage 6 已有的模式字段与 UI 开关之上，把「两种请求策略」从**同一个 `submit()` 内的分支**改为**两条具名执行路径**，并让 Chat 路径在类型与 import 两个层面都无法触达 Agent 基础设施。三个逻辑独立的本地提交 `081e3ea`（core 分类器）、`9c339b9`（先红的验收测试）、`0a02638`（执行路径拆分），**未 push**。
...
- **0.4.0a12 的真实宿主状态**：`--context` **已跑 32/32 PASS**、`recordedRequests = 0`（见上“打包与宿主验证回合”），该 XPI 已装进 `.zotero-chatgpt-dev/context` 树；`--context --native` NOT RUN。该 frozen driver **不点击 Chat/Agent 模式控件**，故 Stage 6 模式控件在真实 dock 的布局/命中区、真实 IME、焦点、滚动锚点与原生缩放仍**未观察**；Chat 模式“结构性只读”未在真实 Zotero/真实库中尝试写操作来证实。**真实模型（`--live`/`--live-model`）NOT RUN**。
...
- **Stage 8（HEAD `0a02638`）无宿主、无发行物、无真实模型证据**：只有代码 + 单元测试（含静态 import 与类型断言）。`package:dev`/`verify:artifacts`/`install:dev`/`verify:install`/`release:dry-run` 与任何 `tests/host/**` 驱动均未运行，`dist/` 仍为 0.4.0a12 字节。两条执行路径仍共用**同一条 Codex 推理传输**（没有第二套模型通道或独立额度记账，未虚构配额）。Chat 档「不触达 Agent 基础设施」是代码结构 + 端口 spy 断言，未在真实 Zotero/真实库中以写操作反证；`core/chat/action-intent.ts` 仅覆盖英文祈使句，中文等其它语言按设计漏判（假阴性）。
