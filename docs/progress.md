...
| --- | --- | --- | --- |
| `8770ffd0ac32439a2f196c1e84a922b8fc2822e3d82ca3f0e5999b4327613f80`（92,675,982 B） | 0.4.0a10 | `dist/`（`SHA256SUMS` 唯一条目）；`.zcr-dev/context/profile/extensions/` | 代码 + 单元 + 产物；**真实宿主 `--context` 32/32 PASS**（2026-09-18，专用 `.zcr-dev/context` 树；重构树：读取/写入分离 + 未发送标签改为 New chat + 产品名改 Zotero GPT Reader）。**从未装进 owner 正常 profile** |
...
- **a10 已有 `--context` 32/32 宿主证据（见上）；`--context --native` NOT RUN**，真实模型（`--live`/`--live-model`）NOT RUN。a10 改动过 `content/zcr.js`（模块搬迁、未发送标签文案、偏好面板标签），宿主证据只覆盖 `--context` 列出的本地路径与 UI 切换。
...
- 已归档报告的磁盘位置（忽略目录）：a10 32/32 `.zcr-dev/verification/scope-2026-09-18-a10/`、a9 32/32 与中途失败 `.zcr-dev/verification/scope-2026-09-15-a9/`、a8 标题失败 `scope-2026-09-15-a8/`、a3 失败 `.zcr-dev/verification/scope-2026-09-13-a3/`、a4 32/32 `scope-2026-09-13-a4/`、a5 32/32 `scope-2026-09-14-a5/`、0.4.0a1 三份 `scope-2026-09-12/`、0.3.0a1 `scope-2026-09-11/`。
