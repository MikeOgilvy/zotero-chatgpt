...
- 本机（macOS，且 `dist/` 存在**当前 manifest 版本**即 a5 对应的 XPI）实测：**`npm run test:unit` → 1068 passed / 79 files / 0 skipped**（2026-09-14 并行会话回合实测）。
- 与上一版 1044 / 79 的差异来自本轮：会话命名改取最小空闲后缀（`tests/core/store.test.ts` 2 条）、presenter 多会话面板与后台事件路由（`tests/zotero/presenter.test.ts` 3 条）、dock 标题条与切换（`tests/zotero/chat-view.test.ts` 3 条）、标题条本地化与样式（`tests/zotero/ui-locale.test.ts` 1 条、`tests/zotero/sidebar-styles.test.ts` 1 条）；另有 2 条既有视图断言随“关闭一个面板落到另一个已打开会话”改写。更早 1044 / 79 的差异来自纸张书目卡片与本地读取状态视图用例、PDF 就绪上限回归（`document-version.test.ts`）、历史直删与偏好面板文案更新，删除死模块后的 `tests/core/workspace-export.test.ts` 一并移除。
