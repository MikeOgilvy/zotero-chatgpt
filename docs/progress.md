...
诚实说明第 3 点：a32 的两次真实 Chat 运行里 `attempts` 都是 1，也就是说这条重发路径**没有被实际触发**。它是针对 a30 观测到的 `not-accepted`（点击后 20 秒内没有出现标记消息）加的有界保护，并用回归测试锁定了“页面未接住时才重发、页面已接住时绝不重发”这两个方向；但它目前只有单元级证据，没有真实服务证据证明它是必需的。
