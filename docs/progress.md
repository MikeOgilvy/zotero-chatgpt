...
- **本轮门禁（Core 执行边界重构，2026-09-19，代码树 `4e06b04` + 本轮文档/版本提交）**：`npm run typecheck` PASS；`npm run lint` PASS；`npm run test:unit` **86 files / 1166 passed / 0 skipped**；`npm run package:dev` → `dist/zotero-chatgpt-0.4.0a15-dev.xpi`（92,684,240 B，SHA-256 `b82be9130aec0b720e1dc71f787218417ff0db716a2644a1ae3025d3bf301054`）；`npm run verify:artifacts` **84 files PASS**。**本次未运行宿主驱动**：真实宿主 `--context`（含模式控件断言）与真实宿主手动路径 NOT RUN；**真实模型（`--live`/`--live-model`）NOT RUN**。本轮删除了旧的核心行为断言并改写了 Codex 协议用例（显式 `mode: 'agent'`），不是新增覆盖：见下“Core 执行边界重构”。
...
**验证（证据分层）**：
- 单元/静态：`npm run typecheck` PASS、`npm run lint` PASS、`npm run test:unit` **86 files / 1166 passed / 0 skipped**。新增 `tests/core/execution-boundary.test.ts`（8 条，在进程边界把 `thread/start`/`thread/resume`/`turn/start`/`turn/interrupt` 当 Codex 入口断点：A 普通 chat、B 带 PDF 的 chat、C 超长复杂 chat、D chat 动作指令均不命中且请求如实 `failed`；E agent 必然命中 `thread/start`+`turn/start`；F agent→chat 不顺带新增任何 Codex 入口、两请求生命周期独立；G 默认 Chat 档打开会话不起线程/轮次；H 中断的 chat 请求记为 `failed` 且不触发 `thread/resume`/`thread/read`，即使该会话已有 Agent 线程）；`tests/core/chat-executor.test.ts`（5 条）、`tests/core/execution-router.test.ts`（4 条）、依赖边界 +1。
...
- 发行物：`npm run package:dev` → `dist/zotero-chatgpt-0.4.0a15-dev.xpi`（92,684,240 B，SHA-256 `b82be9130aec0b720e1dc71f787218417ff0db716a2644a1ae3025d3bf301054`）；`npm run verify:artifacts` 84 files PASS。
