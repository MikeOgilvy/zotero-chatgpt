...
Zotero ChatGPT 当前开发版本是 npm **0.4.0-alpha.1** / Zotero **0.4.0a14**（2026-09-18 重命名为 Zotero ChatGPT 升到 a11 后，7 阶段 Chat/Agent 重构改变了 bundle 字节，按“新字节先升版本”升到 a12；随后 Stage 8 Chat/Agent 执行路径分离再次改动 `content/zchatgpt.js` 与 manifest 字节，升到 a13；同日 Chat/Agent 模式修复改了模式控件渲染、核心边界与 `content/zchatgpt.js` 字节，升到 a14；npm 工作区版本不是侧载身份，未随动）。从仓库根目录执行，Node **24.x**（`.nvmrc` 为 24.11.0）、npm **11.6.1**；最终身份以 package.json、manifest 和实际 XPI 为准。当前目标平台是 macOS Apple Silicon / Zotero 9.0.6。
...
产品模式（Chat Mode / Agent Mode）由[产品规格](zotero-chatgpt-user-flow.md)定义；两种模式共用同一文档上下文层，模式路由尚未成为独立的构建或宿主阶段，本文件的命令不区分模式。
...
`runtime-prepare.mjs` 仅在固定运行资产需要准备时执行：从 manifest 指定的官方归档获取并校验 Codex 0.154.0 到忽略的 `.zotero-chatgpt-dev/runtime-cache/`。它不替换系统 CLI，也不操作认证。缺少真实运行资产时构建失败，不用测试 fixture 冒充发行包。
...
当前目标文件名为 `dist/zotero-chatgpt-0.4.0a14-dev.xpi`。不要在文档多处手写 digest；以 `dist/SHA256SUMS`、实际包身份和 progress 为准。

```sh
npm run verify:install -- build-info \
  --xpi dist/zotero-chatgpt-0.4.0a14-dev.xpi \
...
**当前产品/原生功能验收只用 `.zotero-chatgpt-dev/context/{profile,data}` 和合成材料。** 安装生命周期测试另用下述 s6 隔离树；所有目标都必须是经核对的 `.zotero-chatgpt-dev/` 专用子树。正常 Zotero 可以继续运行。已有 `.zotero-chatgpt-dev/profile`、旧测试账号和其他 Codex 会话都不能被清空、复制认证或当临时垃圾处理。
...
| 准备命令 | 会自动执行的内容 |
| --- | --- |
| `node scripts/prepare-host-test.mjs --context` | 当前 PDF 本地提取、页面标签/范围、Chat/Agent 模式控件（真实 dock 的默认态、选项类、点击切换与回切）、原生 UI/会话/附件切换及合成性能样本；重启会话后还预检原生偏好设置面板（注册身份、真实打开 Preferences 窗口并挂载面板、禁用/启用不叠加面板）且不写业务记录；不发模型请求 |
...
```sh
npm run install:dev -- plan     --profile "<profile 目录>" --xpi dist/zotero-chatgpt-0.4.0a14-dev.xpi
npm run install:dev -- install  --profile "<profile 目录>" --xpi dist/zotero-chatgpt-0.4.0a14-dev.xpi
...
- `revert` 只还原杠杆（不换包）；`rollback` 换回最近的 `.zchatgpt-bak-*` 备份（同样先把当前包另存为备份）。所有子命令加 `--json` 输出结构化结果。

`install:dev` 面向真实 profile；`.zotero-chatgpt-dev/` 隔离树的生命周期测试仍用 `verify:install`，两者不互相代替。

## 人工登录的模型目录实测

`--live-model` 是与 `--context` 并列的独立阶段，只在 `.zotero-chatgpt-dev/live/{profile,data}` 合成树上测量运行时实际解析出的模型目录：选择器中的模型 id、每个可选模型的推理强度档位与速度开关、以及当前选中的模型。报告写入 `.zotero-chatgpt-dev/live/host-report.json`，只含模型 id 与非识别性目录元数据。

该阶段**不发任何模型请求**：没有 `turn/start`、没有问题、没有图像生成，也不读取、复制、记录或截图任何凭据文件。它必须有操作者在场，且只做**一次**由人完成的官方登录——driver 只等待侧栏变为 `signedIn`，绝不点击登录按钮、绝不输入凭据、绝不代替人走 OAuth 流程；等待超时（默认 30 分钟）时以 `login-not-completed` 结束，而不是伪造通过。driver 只被打进 `.zotero-chatgpt-dev/live/` 的隔离测试插件（`zchatgpt-host-test@local`），**从不进入产品 XPI**。

```sh
node scripts/prepare-host-test.mjs --live-model
/Applications/Zotero.app/Contents/MacOS/zotero \
  -no-remote -profile "$PWD/.zotero-chatgpt-dev/live/profile" \
  -datadir "$PWD/.zotero-chatgpt-dev/live/data"
```

`--live-model` 与 `--context`、`--native`、`--live`、`--acceptance` 互斥，也不在 CI 内（需要 macOS GUI 与操作者本人授权）。阶段名与 `.zotero-chatgpt-dev/live/` 树的映射由 `tests/build/host-test-stage.test.ts` 固定；具体模型、档位与账户能力一律以当次报告为准。

## 发行边界

0.4.0a14 是开发预览，`update_url` 仍为 zotero-chatgpt-dev.invalid 占位，未启用公开更新频道。固定 runtime 及第三方库/字体的许可必须随资产保留；项目自身按 MIT 许可发布，正文见根目录 `LICENSE`，`package.json` 的 `license` 字段与之一致。Intel、Windows、Linux 未经过同等验证，不能进入已支持平台声明。
...
推送、公开 Release、发布站点、启用付费新服务和操作真实文献库不在本地开发命令的隐含授权中。`release:dry-run` 只是可审查的本地计划；分支名不构成授权范围，实际提交/发行继续遵循当前会话授权。
