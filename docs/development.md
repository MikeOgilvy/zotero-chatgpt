...
> 文档类型：操作与验证规则。修订日期：2026-09-19。
> 命令和环境基线保留自本次上传的开发文档；本轮没有运行仓库构建或 Zotero。新增 UI 检查是后续开发的验收要求，不是新增脚本已经存在的声明。
...
涉及过期文档时，先迁移仍有效的权限、兼容、测试和未完成事项，再删除旧文件并修复引用。不要保留多份互相冲突的“当前架构”；也不要因旧计划要求“不动 UI”“禁止 agent 目录”等历史阶段限制，拒绝本轮明确的 UI 修复。

不存在完整源码或原始报告时，把相关判断记为待核查。文档说某路径曾存在，不等于它现在仍有缺陷；原文档记录 PASS，不等于本轮重新验证。
...
窄、常规、宽侧栏均应检查。可将 360 / 480 / 720 CSS px 作为本轮测试采样点，但这些是建议采样值，不是已经验证的最小支持宽度；宿主无法达到的尺寸记录实际值。
