...
**Spec:** [模块设计](../../module-design.md)、[项目决策](../../project-decisions.md)、[用户流程](../../zotero-codex-user-flow.md)、[接口约定](2026-09-08-zotero-codex-reader-contracts.md)、[验收记录](../../qa/acceptance-v0.1.md)。
...
**文件重点：** `packages/core/src/codex/{reader-policy,transport,models,errors}.ts`；`packages/core/src/sessions/{service,store,log}.ts`；`packages/zotero/src/runtime/{supervisor,process,storage}.ts`；runtime 清单、Codex 资产构建、JSONL/运行时测试。登录入口在侧栏 presenter，没有独立 account controller。
...
- [x] 在第一条真实合成请求前，验证最小 requestId、一次活动请求限制、提交日志写入顺序及 stdout 故障后的 uncertain 状态。（`tests/core/log.test.ts`、`tests/core/client.test.ts`、`tests/core/recovery.test.ts`；宿主重启后以相同 ID/终态恢复且不重发）
...
**文件重点：** `reader/{selection,selection-actions}.ts`；`chat/{draft,presenter,view}.ts`；`core/src/sessions/{service,store,log}.ts`；selection/draft/request 测试。

- [x] 单页选区生成不可变引用：原文、附件 key、页码标签、页面索引、位置和可用元数据。（`tests/zotero/selection.test.ts`；宿主 `selection-popup-and-action-bar-appear`）
- [x] 正常选区上方显示 More details / Ask in sidechat，下方原生标注面板保持可用。（宿主 `action-bar-above-native-popup-inside-view`、`native-annotation-panel-intact`）
- [x] More details 自动打开侧栏并提交一次解释；Ask 仅放入引用、聚焦输入框，用户发送后才请求模型。（宿主 Ask/More details 检查；解释已提交，真实回复因限额未观察到）
- [x] 当前附件绑定真实 conversation/thread。（宿主绑定与重启恢复通过；同一对话的第二/第三次追问因限额 NOT RUN）
- [x] 实现基础内容保存、重复点击去重和全局生成生命周期；关闭侧栏继续接收，重新打开恢复已保存内容。（单元去重；宿主 close/reopen 与 disable/enable）
- [x] A 生成时切到 B，不把增量显示或保存进 B；关闭/切换后不读取过期选区。（宿主 sibling isolation 与 switch-back）
- [x] 进程异常后明确不确定状态，保留问题；尚未完成自动对账时也禁止自动重发。（单元 uncertain；宿主重启后不重发。完整崩溃对账在 S5）

**交付标准：** 用户能实际读一段合成论文并连续讨论。这个阶段已经有可试用主功能；当前主要展示可为纯文本，完整公式排版和所有边缘布局在 S4 完成。

**2026-09-09 状态：** 见 [S3 QA](../../qa/s3.md)。23/23 已执行宿主检查通过；真实回复与追问停止因测试账户的 Codex 周限额用尽（2026-09-15 恢复）保持 NOT RUN。限额恢复后重跑 `node scripts/prepare-host-test.mjs --s3` 即可补齐。
...
- [x] 底部提供模型、速度和推理强度；从真实目录加载，验证所有支持的组合与默认值语义。（2026-09-09：composer 与目录对齐已有单元测试；宿主 A23 未跑）
- [x] 同一对话修改设置，下一条请求生效；正在生成的请求快照不变，已有上下文保留。（2026-09-09：presenter 单元测试覆盖；宿主 A24 未跑）
- [x] 加入 Markdown/KaTeX、净化、复制回答、长公式滚动、流式渲染合并和“有新内容”提示。（2026-09-09：单元测试覆盖净化/KaTeX/复制/滚动策略；KaTeX 字体随包；宿主未跑）
- [ ] 完成原生主题、字体/间距、菜单、滚动条、焦点与中文输入法行为。（焦点/IME/灰阶控件已有；宿主视觉未验收）
- [x] 完成可拖动侧栏宽度、窗口变窄、手动缩放优先和关闭后的恢复；PDF 选择/标注仍对齐。（2026-09-09：单元测试 + 宿主 A27 全档位与手动缩放；A28 1024/1440 PASS，800 CSS px 因 macOS 无法再缩小而 NOT RUN）
- [x] 完成选区上方操作条的四边避让、原生标注面板翻转、重新选择、滚动/缩放时失效与重定位。（2026-09-09：单元测试 + 宿主 A30 上/左/右 PASS；底边因合成 PDF 滚到底无可见 span 而 NOT RUN）

**交付标准：** 用户描述的主要操作与视觉细节完整；基础布局已在 S1 证明，这里处理完整体验，而不是此时才首次发现宿主不能真正停靠。

**2026-09-09 状态：** composer、回答渲染、拖宽策略和选区四边避让已在源码与单元测试中落地。`--s4` 宿主在 XPI `c1b898ac…` 上 **40/40 已执行检查通过、5 NOT RUN**（含 A27 缩放矩阵、A28 1024/1440、笔记区让出、A30 上/左/右、Ask 不发送）。A26 主题 pref 不可读、800px 窗口、底边选区、A23/A24 发送未跑。**未向 Codex 发送。**

## 8. S5：历史管理与完整故障恢复

**涉及模块：** M3/M5/M6 和对应 UI。**对应细则：** T6/T8/T9 的恢复部分。

- [x] 增加每附件多对话、当前对话切换、历史列表和独立草稿恢复。（2026-09-09：`store.select` + `ReaderClient.select` + presenter 草稿分会话 + 侧栏「对话历史」）
- [x] 完成视图订阅/快照的 seq 去重，开关侧栏或重新挂载不遗漏增量。（2026-09-09：presenter 先 subscribe 再 `get()`；仅应用 `seq > lastSeq`；核心 remount 测试不触发新的 `turn/start`）
- [x] 完成 Codex 进程重启后的握手、resume、历史对账；确认终态后解除活动请求状态。（2026-09-09：先 `thread/resume` 再 `thread/read`；accepted 只派发一次；未登录时打开会话不会把恢复标成已完成，登录后再对账；completed/interrupted/failed/inProgress 按上游历史恢复；resume 失败则保持 uncertain）
- [x] uncertain 无法核实时隔离旧会话；用户明确新开对话才能重发，不凭超时推断已结束。（2026-09-09：`BUSY` + 侧栏提示；超时不把 uncertain 改成终态）
- [x] 覆盖取消/完成竞态、派发中取消、半条 JSON、损坏日志尾、历史文件损坏和 schema 变化。（2026-09-09：`tests/core/recovery.test.ts`、`tests/core/log.test.ts`、`tests/core/store.test.ts`）
- [x] 完善私有数据与诊断分离、存储位置与备份/恢复说明。（2026-09-09：`shareableDiagnostics` 白名单；侧栏「复制诊断」只复制该 JSON；`docs/development.md` 记录位置与清除说明。）

**交付标准：** 长时间使用、重启 Zotero、子进程失败和会话切换都有可解释的恢复路径；基础去重从 S2/S3 已存在，这里补齐系统性的故障矩阵。

**2026-09-09 状态：** 对话切换、seq 去重、进程重启 resume→read 对账、登录后再恢复、uncertain 隔离、崩溃矩阵和可分享诊断 UI 已打进当前开发 XPI。`--s5` 宿主 17/17 已执行通过：TERM 自有进程后错误并重开握手、同一对话、停用收尾、注入 uncertain leftover 显示隔离且不自动发送。进行中的 turn 被杀后的 resume **NOT RUN**（限额，未 `turn/start`）。

## 9. S6：完整测试与发行候选包

**涉及模块：** 全部模块，重点 M7。**对应细则：** T10/T11。

- [ ] 将已完成阶段的自动化与宿主证据整理到 QA 记录，完整执行 A01–A30；没有执行的用例保持 NOT RUN。（2026-09-09：已汇编 [acceptance-v0.1.md](../../qa/acceptance-v0.1.md)，未在本阶段重跑宿主矩阵）
- [ ] 每个真实缺陷用有区分力的回归测试覆盖，修复后重跑受影响路径。
- [x] 源码侧增加无凭据的 PR CI（typecheck/lint/test:unit）和 `verify:artifacts`（生产 bundle 无 `['"]node:` 导入、无 auth/论文记录、含 Codex 与第三方 LICENSE/NOTICE；XPI 若旁有 `SHA256SUMS` 则核对摘要）。从当前工作树副本（非 `git HEAD`）重建了相同摘要的 macOS arm64 a1 XPI；干净 `HEAD` checkout 重建仍因禁止提交而未执行。
- [x] 源码侧增加本地安装生命周期 CLI（`npm run verify:install`：virgin profile 布局、本地 XPI 升级/回退且保留 records、拒绝 GitHub Release URL 与已登录 `.zcr-dev/profile|data`、含空格/中文路径解包、由 SHA256SUMS 生成占位 `updates.json`/`build-info.json`、对现有开发 XPI 内 Codex 二进制 `codesign --verify --strict`）。
- [x] 专用 `--s6` 在 `.zcr-dev/s6-virgin/` 上用 AddonManager 安装现有本地开发 XPI：握手、`signedOut`、停用后同版本替换且 records 保留；不发送。GitHub 下载和无系统 Node 仍待执行。
- [ ] 从实际下载的 XPI 在无系统 Node/CLI 的干净环境完成安装、授权、选区聊天；检查原生签名/隔离属性和路径问题。
- [x] 演练完整 XPI 升级、原生进程收尾、旧版本回退和数据保留。（2026-09-09：同版本宿主替换 + Node fixture a1→a2 + `.zcr-dev/s6-upgrade/` 上两个已打包 XPI 的 AddonManager 升级/回退，不发送）
...
- [x] 源码侧准备 CONTRIBUTING、CHANGELOG、`docs/release.md`、issue/PR 模板和 `npm run release:dry-run`（拒绝 `--publish`）。LICENSE 与中英文 README 发行稿仍待作者确定。
- [x] PR CI 执行无凭据的 typecheck/lint/test:unit；`Release dry-run` workflow 不上传、不调用 `gh release`。完整平台 XPI 构建不在 Ubuntu CI。
