...
function taskPort(conversationId: string) {
  ...
  const port: AgentTasks = { list: () => Promise.resolve(copy(tasks)), get: id => Promise.resolve(copy(tasks.find(task => task.id === id)!)), subscribe: () => () => {}, planAnnotations: vi.fn<AgentTasks['planAnnotations']>(input => { const task: AgentTaskRecord = { ...base(), question: input.question, kind: 'annotations', paper: input.paper, documentRevision: input.revision, ...(input.modelRequestId ? { modelRequestId: input.modelRequestId } : {}), items: [] }; tasks.push(task); return Promise.resolve(copy(task)); }), planAcquisition: vi.fn<AgentTasks['planAcquisition']>(input => { const task: AgentTaskRecord = { ...base(), question: input.question, kind: 'acquisition', target: input.target, items: [] }; tasks.push(task); return Promise.resolve(copy(task)); }), approve: vi.fn<AgentTasks['approve']>(id => Promise.resolve(copy(tasks.find(task => task.id === id)!))), cancel: vi.fn<AgentTasks['cancel']>(id => Promise.resolve(copy(tasks.find(task => task.id === id)!))), reconcile: vi.fn<AgentTasks['reconcile']>(id => Promise.resolve(copy(tasks.find(task => task.id === id)!))), undo: vi.fn<AgentTasks['undo']>(id => Promise.resolve(copy(tasks.find(task => task.id === id)!))) };
  return { port, tasks };
}
