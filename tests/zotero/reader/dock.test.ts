...
function readerDocument(): Document {
  ...
  doc.body.innerHTML = [
    ...
    '<div id="zotero-context-pane"><item-details class="zcr-chat-active"><item-pane-custom-section data-zcr-section data-pane="codex-reader"><div data-type="body"></div></item-pane-custom-section></item-details></div>',
  ].join('');
  return doc;
}
...
it('mounts the chat dock inside reader content below the toolbar, not in a context-pane header', () => {
  ...
  expect(split.classList.contains('zcr-dock-open')).toBe(true);
  expect(toolbar.contains(dock)).toBe(false);
  expect(context.contains(dock)).toBe(false);
  expect(context.contains(sidebar)).toBe(false);
  expect(doc.querySelector('#reader-ui .zcr-chrome, .toolbar .zcr-chrome')).toBeNull();
  expect(sidebar.classList.contains('zcr-paper')).toBe(true);
  expect(dock.classList.contains('zcr-paper')).toBe(true);
...
it('shrinks the PDF column with a real remaining width instead of covering the page', () => {
  ...
  expect(doc.documentElement.style.getPropertyValue('--zcr-dock-width')).toBe('400px');
  expect(split.classList.contains('zcr-dock-open')).toBe(true);
  ...
  expect(split.classList.contains('zcr-dock-open')).toBe(false);
  expect(doc.documentElement.style.getPropertyValue('--zcr-dock-width')).toBe('');
});
...
it('opens NativeReaderPane chat in the reader iframe column and hides the ItemPane Codex section', async () => {
  ...
  const dock = readerDoc.querySelector('[data-zcr-dock]');
  const sidebar = readerDoc.querySelector('[data-zcr-sidebar]');
  const toolbar = readerDoc.querySelector('.toolbar')!;
  expect(context.collapsed).toBe(true);
  expect(readerDoc.querySelector('[data-zcr-sidebar-css]')?.textContent).toMatch(/writing-mode:\s*horizontal-tb/u);
  expect(mainDoc.querySelector('[data-zcr-sidebar-css]')).toBeNull();
  ...
  expect(mainDoc.querySelector('#zotero-context-pane [data-zcr-sidebar]')).toBeNull();
  expect(mainDoc.querySelector('item-details.zcr-chat-active')).toBeNull();
  expect(sidebar?.classList.contains('zcr-paper')).toBe(true);
  pane.controller.close();
  expect(readerDoc.querySelector('[data-zcr-dock]')).toBeNull();
...
it('paints sidebar chrome with the reader paper background, not grey sidepane fill', () => {
  ...
  expect(css).toMatch(/\.zcr-paper[\s\S]*--material-background/u);
  expect(css).not.toMatch(/\.zcr-paper[\s\S]{0,200}--material-sidepane/u);
  expect(css).toMatch(/#split-view\s*>\s*\.zcr-dock|#split-view \.zcr-dock/u);
  expect(css).toMatch(/#split-view\.zcr-dock-open[\s\S]{0,120}flex-direction:\s*row/u);
  expect(css).toMatch(/#split-view\.zcr-dock-open \.primary-view[\s\S]{0,80}min-width:\s*0/u);
});
...
it('attaches the dock stylesheet to the reader iframe document and keeps a composer-width column against reader smash rules', () => {
  ...
  expect(sheet?.textContent ?? '').toMatch(/\.zcr-dock[^{]*:where\(\s*button/u);
  expect(sheet?.textContent ?? '').toMatch(/\.zcr-dock[^{]*:where\([^)]*\bp\b/u);
  expect(sheet?.textContent ?? '').toMatch(/\.zcr-dock[^{]*:where\([^)]*\bsection\b/u);
  expect(sheet?.textContent ?? '').toMatch(/\.zcr-dock[^{]*:where\([^)]*\baside\b/u);
  ...
  const sidebar = renderReaderShell(body, { title: 'Paper', key: 'PDFONE01', libraryID: 1 }, () => undefined);
  expect(sidebar.classList.contains('zcr-sidebar')).toBe(true);
