...
it('releases a paused two-pass core batch on cancellation and starts the ordinary core queue', async () => {
  ...
  const client = await createReaderClient(server.p, f.storage, { codexVersion: '0.144.1', cwd: '/isolated', uuid: f.clock.uuid, now: f.clock.now, pluginVersion: '0.4.0-batch-regression' });
