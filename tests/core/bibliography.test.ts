...
} from '../../packages/core/src/context/bibliography.ts';
...
describe('bibliographyBlock', () => {
  ...
  it('drops a field that is absent, empty or only whitespace instead of emitting an empty label', () => {
    expect(bibliographyBlock({ title: 'T', authors: [], volume: '', issue: '   ', publisher: '\n' })).toBe('Title: T');
  });
  ...
  it('never returns a blank line, so it can be joined into a larger message', () => {
    const block = bibliographyBlock({ title: 'A\n\nB', authors: ['x'] });
    expect(block).not.toContain('\n\n');
    expect([...block].every(character => character !== '\r')).toBe(true);
  });
  ...
  it('can omit the abstract for a caller that already sends it in a structured field', () => {
    const withAbstract = bibliographyBlock(full);
    const without = bibliographyBlock(full, { includeAbstract: false });
    expect(without).toBe(withAbstract.split('\n').filter(line => !line.startsWith('Abstract: ')).join('\n'));
    expect(without).not.toContain('We study nothing.');
    expect(bibliographyBlock({ title: 'T', authors: [], abstractNote: 'Only an abstract' }, { includeAbstract: false })).toBe('Title: T');
  });
