...
async function setup(configure?: (fixture: ReturnType<typeof server>) => void) {
  ...
  let id = 0;
  const client = await createReaderClient(fixture.p, new MemoryStorage(), { codexVersion: '0.144.1', cwd: '/isolated', uuid: () => `12345678-0000-4000-8000-${String(++id).padStart(12, '0')}`, pluginVersion: '0.4.0-runtime-test', now: () => '2026-09-12T10:00:00.000Z' });
