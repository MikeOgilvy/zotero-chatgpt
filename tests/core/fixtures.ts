...
export const thread = { id: 'thread-1', sessionId: 'session-1', forkedFromId: null, parentThreadId: null, preview: '', ephemeral: true, modelProvider: 'openai', createdAt: 1, updatedAt: 1, recencyAt: null, status: { type: 'idle' }, path: null, cwd: '/isolated', cliVersion: '0.144.1', source: 'appServer', threadSource: null, agentNickname: null, agentRole: null, gitInfo: null, name: null, turns: [] };
...
export function server() {
  ...
  handlers.set('thread/start', () => threadResponse);
  handlers.set('turn/start', () => ({ turn }));
  ...
  p.onWrite = m => {
    ...
    if (result !== undefined) p.emit({ id: m.id, result });
  };
  return { p, handlers };
}
