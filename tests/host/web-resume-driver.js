...
async function runHostSmoke(config) {
  ...
  try {
    ...
    let stableReadiness = 0;
    const preSubmit = await until(async () => {
      try {
        const [product, observer] = await Promise.all([
          boundedQuery(browser, 'ZoteroChatGPTOfficialChat', 'probe', {}, 5000),
          boundedQuery(browser, 'ZoteroChatGPTWebAcceptance', 'probe', { verificationToken: config.webResumeToken }, 5000),
        ]);
        const state = { productStatus: product?.status ?? 'invalid-response', inputReady: observer?.inputReady === true, sendReady: observer?.sendReady === true, draftLength: Number(observer?.draftLength ?? 0), canonicalURL: observer?.canonicalURL ?? null, bridgeReady: String(browser.getAttribute('data-zchatgpt-bridge-ready') || '') };
        const ready = ['ready', 'composer-ready'].includes(state.productStatus) && state.inputReady && state.canonicalURL === targetURL && state.bridgeReady !== 'restoring'; stableReadiness = ready ? stableReadiness + 1 : 0; report.preSubmitReadiness = { ...state, stableSamples: stableReadiness }; await save(); return stableReadiness >= 2 ? state : null;
      } catch (error) { const text = String(error?.message ?? error); if (/WindowGlobal unavailable|timed out|NS_ERROR_NOT_INITIALIZED|dead object|actor/iu.test(text)) { stableReadiness = 0; return null; } throw error; }
    }, 'stable-official-composer-before-stop-test', 60000);
    ...
    report.submissionAttempts = 1; await save(); const submitted = await boundedQuery(browser, 'ZoteroChatGPTOfficialChat', 'submitQuestion', { question: stopQuestion }, 90000); report.productSubmitStatus = submitted?.status ?? 'invalid-response'; report.productSubmitAttempts = Number.isInteger(submitted?.attempts) ? submitted.attempts : null; await save();
    if (submitted?.status !== 'accepted') {
      const allowedReasons = ['context-changed', 'context-disabled', 'context-empty', 'context-failed', 'invalid-response', 'busy', 'draft-changed', 'composer-missing', 'submit-missing'];
      report.productSubmitBlockedReason = allowedReasons.includes(submitted?.reason) ? submitted.reason : submitted?.reason ? 'other' : null;
      report.status = submitted?.status === 'blocked' ? 'blocked' : 'failed'; report.outcomeUnknown = submitted?.status !== 'blocked'; report.preSubmitState = preSubmit; report.finishedAt = new Date().toISOString(); await save(); return;
    }
    report.preSubmitState = preSubmit;
