...
async function runHostSmoke(config) {
  ...
  try {
    ...
    const annotationCard = await until(() => taskCard('Annotations'), 'annotation-review', 60000);
    await check('annotation-review-before-write', annotationCard.dataset.state === 'review' && annotationCard.querySelectorAll('[data-zchatgpt-task-item-id]').length === 5 && !annotationCard.querySelector('[data-zchatgpt-task-action="approve"]').disabled && attachment.getAnnotations().length === annotationsBefore);
    ...
    const organizationCard = await until(() => taskCard('Organize library'), 'organization-review', 60000); await Promise.all([parent.loadAllData(), peer.loadAllData()]);
    await check('organization-frozen-review-before-write', organizationCard.dataset.state === 'review' && organizationCard.querySelectorAll('[data-zchatgpt-task-item-id]').length === 2 && organizationCard.textContent.includes(parent.getField('title')) && organizationCard.textContent.includes(peer.getField('title')));
